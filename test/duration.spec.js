...
test.group('duration', () => {
  test('a valid duration object should not throw an exception',
    async (assert) => {
      let hasError;
  ...
  test('an object with invalid fields should throw an exception',
    async (assert) => {
      const hasError = await throws(async () => duration({
        data: {
          seconds: 2, years: 3, probably_error: ':)',
        },
      }, 'data'));
