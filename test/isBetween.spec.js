...
test.group('isBetween', () => {
  test('a number inside the given range should not throw error', async (assert) => {
    let hasError;
  ...
  test('a number outside the given range should throw an error', async (assert) => {
    let hasError;
  ...
  test('args can be strings representing floats', async (assert) => {
    let hasError;
