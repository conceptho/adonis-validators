...
test.group('only', () => {
  test('an object with correct keys should not throw an exception',
    async (assert) => {
  ...
  test('an object with incorrect keys should throw an exception',
    async (assert) => {
      const data = {
        a: 1, b: 2, c: 3,
      };
  ...
  test('values which are not objects should throw error',
    async (assert) => {
      let hasError;
